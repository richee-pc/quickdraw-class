...
# ------------------------------------------------------------------ 수집기 체험
elif page == "🧩 나만의 퀵드로우 수집기":
    st.title("🧩 나만의 퀵드로우 수집기")
    st.subheader("직접 데이터를 만들며 학습과 편향의 의미를 먼저 체험해요")
    st.markdown(
        """
        이 활동은 본격적인 파이썬 실습 전에 진행하는 **사전 체험 단계**입니다.
        학생이 먼저 한 물체(예: 자동차)를 자유롭게 그려 데이터를 모으고,
        이후 전체 학생 데이터 분포를 함께 보며 **데이터 다양성과 편향의 영향**을 확인합니다.
        """
    )
    ...
    st.markdown(
        ...
        - **데이터 편향의 영향**: 학생들이 무의식적으로 비슷한 형태만 많이 그리면 성능이 흔들릴 수 있습니다.
        - **토의 포인트**: 우리 반 전체 그림을 모아 보니 어떤 형태가 많이 보였나요?
        """
    )

    st.markdown("### ✅ 수업 진행 체크리스트")
    c1, c2 = st.columns(2)
    with c1:
        st.markdown(
            """
    ...
    with c2:
        st.markdown(
            """
            4. 교사가 DB에서 전체 데이터를 불러와 분포 공유  
            5. 다시 테스트해서 변화 비교  
            6. "왜 성능이 달라졌는지" 한 줄 정리
            """
        )

    st.caption("학생 배포용 화면으로 구성되어 있습니다. (교사용 데이터 관리 UI는 표시하지 않음)")

    with st.expander("교사용 빠른 멘트(데이터 편향 설명)"):
        st.markdown(
            """
            "AI는 우리가 준 데이터 범위 안에서만 똑똑해집니다.
            한쪽 그림만 많이 보여주면 그쪽에 맞게만 학습해요.
            그래서 현실에서 잘 쓰려면 다양한 상황의 데이터를 균형 있게 모아야 해요."
            """
        )
